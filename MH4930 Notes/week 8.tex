...
\begin{defn} [Homomorphism of (co)chain complexes]
    Let $(X^\bullet, d^\bullet)$ and $(Y^\bullet, \delta^\bullet)$ be two (co)chain complexes. The homomorphism from $X^\bullet$ to $Y^\bullet$ is a collection of module homomorphism $\varphi:X^\bullet \to Y^\bullet$ where for all $n$ we have $\varphi_n:X^n \to Y^n$ such that the following diagram commutes:
...
\begin{pro}
    Let $\varphi:X^\bullet \to Y^\bullet$ be a homomorphism of cochain complex. Then it induces an $R$-module homomorphism $\alpha^*:H^n(X) \to H^n(Y)$, one for each $n$, given by $\overline x \mapsto \overline {\varphi(x)}$. The statement holds similarly for chain complexes.
\end{pro}
...
\begin{proof}
    ...
    So $\varphi_n(x) \in \ker \delta_{n+1}$. Thus an element $\overline{x}\in H^n(X)$ means that $x\in \ker d_n$, implying that $\varphi_n(x) \in \ker \delta_{n+1}$, and thus $\overline{\varphi_n(x)}\in H^{n+1}(X)$. So the claimed map is valid.

    To show that the map is well-defined, consider elements $\overline{x}$ and $\overline{x'}$ from $H^n(X)$. This implies that $x,x'\in \ker d_n$. Consequently $x-x'\in \ker d_{n+1}\subseteq \im d_n $ by the definition of complexes, so let $x''\in X^{n-1}$ such that $d_n(x'') = x-x'$. Observe that
...
\begin{defn} [Projective resolution]
    ...
    such that each $P_i$ is a projective $R$-module. In shorthand notation we write $P_\bullet \twoheadrightarrow V$ to denote a projective resolution of $V$.
\end{defn}
 
\medskip

\begin{re}
    Similarly we can define a free resolution, where we replace the projective property by free.
\end{re}
...
\begin{proof}
    ...
    Let $\ep$ be $d_0$, and by our construction we observe that $P_n$ surjects to $\ker d_{n-1}$ via $d_n$, i.e.
...
\begin{re}
    ...
    Also, projective resolution is not unique. Consider:
    \[0 \to V \xto{\alpha} V\oplus V \xto{\beta} V \to 0 \to \dots\]
    where $\alpha:v\mapsto (v,0)$ and $\beta:(v,w)\mapsto w$. This is also a projective module of $V$
\end{re}
...
\begin{defn}[Ext group]
    ...
    \[\mathcal C:=0 \to \h_R(P_0, W)\xto{d_1^*} \h_R(P_1, W) \xto{d_2^*} \h_R(P_2, W)\xto{d^*_3}\dots\]
    where $\h_R(V,W)$ is forgetted. It is indeed a complex since
    \[d_{n+1}^*\circ d_n^* = (d_n \circ d_{n+1})^* = 0\]
    Note that this complex is usually not exact.

    The $n$-th Ext group is defined as the $n$-th cohomology group of the above complex: 
...
\begin{proof}
    ...
    By applying the 1st Isomorphism Theorem on $\ep^*$ we get 
    \[\frac{\h_R(V,W)}{\ker \ep^*}\cong \im\ep^* = \ker d_1^*\]
    where from exactness we get $\im \ep^* = \ker d_1^*$. Note that $\ker \ep^* = \im 0 = 0$ due to exactness. Also, by definition of Ext we have that $\ext^0_R(V,W) = \ker d_1^*$. Altogether, we see
