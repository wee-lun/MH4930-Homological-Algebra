\begin{ex}
    ...
    \begin{enumerate}
        ...
        Thus we know 
        \[\ext_\Z^0(\Z/m\Z, D)\cong \h_\Z(\Z/m\Z, D) \cong \sbr{d\in D:m\cdot d = 0} =:\ _m D\]
        To investigate the general case, consider the projective resolution of $\Z/m\Z$:
        \[\dots \to 0\to \Z\xto{\times m}\Z \xto{\bmod m} \Z/m\Z \to 0\]
        where exactness can be verified easily. Taking $\h_\Z(\Z/m\Z,-)$ we get
        \[0\to \h_\Z(\Z,D) \xto{(\times m)^*}\h_\Z(\Z,D)\to 0 \to \dots\]
        And it is clear that $\ext^n_\Z(\Z/m\Z, D)=0$ for all $n\geq 2$. We compute the $\ext^1$ as follow: note $\h_\Z(\Z, D)\cong D$ with isomorphism given by $\varphi\mapsto \varphi(1)$. Thus:
    ...
    \end{tikzcd}\]
    and thus $\ext_\Z^1(\Z/m\Z, D)\cong D/mD$.
    \item If $\Z/m\Z$ is a $(\Z/d\Z)$-module for some $d$, then $\overline{d} = d\cdot \overline{1}= \overline{0}\in \Z/m\Z$, so $m\mid d$. We now compute $\ext_{\Z/d\Z}(\Z/m\Z, D)$ where $D$ is a $\Z/d\Z$-module. To come up with a free resolution of $\Z/m\Z$, note we can start from 
    \[\dots \xto{\alpha} \Z/d\Z \xto{\beta} \Z/d\Z \xto{\alpha} \Z/d\Z \xto{\pi} \Z/m\Z \to 0\]
	where $\pi$ is the canonical surjection. We have to determine what is $\alpha$ and $\beta$. Note we must have $\im\alpha = \ker \pi$. This says that $\alpha$ is the operation $\times m$. Similarly, one can derive that $\beta$ is the operation $\times \frac{d}{m}$. Thus we obtain the free resolution of $\Z/m\Z$:
	 \[\dots \xto{\times m} \Z/d\Z \xto{\times \frac{d}{m}} \Z/d\Z \xto{\times m} \Z/d\Z \xto{\times \frac{d}{m}} \Z/d\Z \xto{\times m} \Z/d\Z \xto{\pi} \Z/m\Z \to 0\]
	 ...
	 We have verified previously that the $0$-th Ext group is isomorphic to $\h_{\Z/d\Z}(\Z/m\Z, D)$. Let $\varphi\in \h_{\Z/d\Z}(\Z/m\Z, D)$. It suffices to check the case for $d:= \varphi(\overline{1})$. Note $\overline{0} = \varphi(\overline{m}) = m \varphi(\overline{1}) = md$. This says that 
	 \[\ext_{\Z/d\Z}^0(\Z/m\Z, D)\cong \sbr{d\in D: m\cdot d = 0}\]
	 Next, since the maps of the complex alternates, all odd-order Ext groups are the same, simialrly for the even-order maps. Direct computation says that if $n$ is odd
...
\begin{re}
    The above example is not rigorous enough, since we have assumed that the $\ext$ group is independent of the choice of projective resolution. The above example is not solid before we establish this fact in the following statement.
\end{re}

\medskip

\begin{pro} [Comparison Theorem]
    Let $f:V\to V'$ be an $R$-module homomorphism and $P_\bullet\twoheadrightarrow V$ be a projective resolution of $V$ and $P'_\bullet$ be an exact complex that surjects onto $V'$, where it need not to be a projective resolution of $V'$. Then there exists $f_n:P_n\to P_n'$ such that the following commute:
    \[\begin{tikzcd}[sep=small]
...
\begin{proof}
	...
	\end{tikzcd}\]
	Note $\delta_0$ is surjective. Also $P_0$ is mapped to $V'$ via $f\circ d_0$. We thus have the following commutative diagram:
	\[\begin{tikzcd}[sep=small]
	&& {P_0} \\
	\\
	{P'_0} && {\im \delta_0 = V'}
	...
	\end{tikzcd}\]
	where the existence of $f_0$ is ensured by the projectivity of $P_0$. This proves the base case.
	...
	\end{tikzcd}\]
	where existence of $f_n$ is ensured by the projectivity of $P_n$. This proves the first part of the statement.
	...
	\end{tikzcd}\]
	where existence of $s_0$ is ensured by the projectivity of $P_0$. We see that the required condition $f_0 - g_0 = s_{-1}d_0 + \delta_1s_0$ holds since the commutativity of the diagram says that $f_0 - g_0 -s_{-1}d_0 = \delta_1s_0$. 
	...
	\begin{align*}
		...
		&= 0
	\end{align*}
	Thus, we have the followng commutative diagram:
	\[\begin{tikzcd}[sep=small]
	&& {P_n} \\
	\\
	...
	\end{tikzcd}\]
	where the existence of $s_n$ is ensured by the projectivity of $P_n$. This concludes the proof.
\end{proof}
...
\begin{proof}
    ...
    For the second statement, if $X^\bullet$ and $Y^\bullet$ are homotopy equivalent, it means having $f:X^\bullet\to Y^\bullet$ and $f':Y^\bullet\to X^\bullet$ such that $f\circ f' \simeq \id_{Y^\bullet}$ and $f'\circ f \simeq \id_{X^\bullet}$, which implies that 
	\[(f\circ f')^* = (\id_{X^\bullet})^* \quad \text{and} \quad (f'\circ f)^* = (\id_{Y^\bullet})^*\]
	Moreover, the maps $f$ and $f'$ induce $f^*:H^n(X^\bullet) \to H^n(Y^\bullet)$ and ${f'}^*:H^n(Y^\bullet) \to H^n(X^\bullet)$ respectively. Lastly, note
...
\begin{thm}
    The $n$-th $\ext$ group $\ext^n_R(V,W)$ is independent, up to isomorphism, of the choice of the projective resolution of $V$.
\end{thm}
\begin{proof}
	...
	\end{tikzcd}\]
	where note $(g_n f_n)^* = f_n^* g_n^*$. Note that the relation $gf-1 = ds + sd$ implies that $f^*g^* - 1^* = s^*d^* + d^*s^*$. By definition of homotopic, this is saying that $f^*\circ g^* \simeq 1^* = \id_X$. Similarly, one can repeat all the arguments above to conclude that $g^*\circ f^* \simeq \id_Y$. Therefore the two complexes involving Hom induced from $P_\bullet\twoheadrightarrow V$ and $P'_\bullet$ are homotopy equivalent. Previous proposition says that the cohomologies computed from these two complexes are isomorphic, and thus the Ext group are independent of the choice of the starting projective resolution.
\end{proof}
...
\begin{thm}
    Let $0\to X\to Y\to Z\to 0$ be a SES of $R$-modules and $D$ be any $R$-module. Then we have a LES of abelian groups 
...
\begin{proof}
	...
	\end{tikzcd}\] 
	where existence of $\varphi$ is ensured by the injectivity of $Q$. Lastly, we check that $d_n^*(\varphi) = \phi$. The commutative diagram implies $\varphi \circ \tilde{d_n} = \tilde{\phi}$, so 
